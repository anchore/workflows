...
class TestValidateSafeString:
    ...
    def test_rejects_leading_dash(self, value: str):
        """Leading dashes could be reinterpreted as CLI flags by anything
        that consumes the value positionally — defense in depth even though
        we currently always pass via `-c key=value`."""
        with pytest.raises(ValidationError) as exc_info:
            validate_safe_string(value, "test_field")
        # could fail either at the regex check or the explicit dash check
        msg = str(exc_info.value)
        assert "dash" in msg or "invalid characters" in msg
...
class TestValidateDeployKey:
    ...
    def test_rejects_concatenated_keys(self):
        """A blob with two private key blocks would be loaded by ssh-add as
        multiple identities, defeating the 'we know which key we're using'
        invariant. Validation must reject this."""
        key = (
            "-----BEGIN OPENSSH PRIVATE KEY-----\n"
            "first key body\n"
            "-----END OPENSSH PRIVATE KEY-----\n"
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate_deploy_key(key)
        assert "exactly one private key block" in str(exc_info.value)

    def test_rejects_mixed_concatenated_keys(self):
        """Same as above but with two different key algorithms — the
        first/last header check would otherwise pass since BEGIN OPENSSH
        and END RSA differ."""
        key = (
            "-----BEGIN OPENSSH PRIVATE KEY-----\n"
            "first\n"
            ...
            "-----BEGIN RSA PRIVATE KEY-----\n"
            "second\n"
            "-----END OPENSSH PRIVATE KEY-----"  # intentionally mismatched end
        )
...
class TestSetupSshKey:
    ...
    def test_ssh_agent_failure_raises_git_error(self):
        """ssh-agent failing must surface as GitError so main() reports it
        cleanly, not as an uncaught CalledProcessError traceback."""
        with patch("subprocess.run") as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(
                returncode=1,
                cmd=["/usr/bin/ssh-agent", "-s"],
                output="",
                stderr="ssh-agent: command not found",
            )
            test_key = "-----BEGIN TEST-----\ntest\n-----END TEST-----"
            with pytest.raises(GitError) as exc_info:
                with setup_ssh_key(test_key):
                    pass
            assert "ssh-agent failed" in str(exc_info.value)
            assert "command not found" in str(exc_info.value)

    def test_ssh_add_failure_raises_git_error(self):
        """ssh-add rejecting a malformed key must surface as GitError. The
        key itself must NEVER appear in the error message."""
        mock_agent_output = (
            "SSH_AUTH_SOCK=/tmp/ssh-mock/agent.12345; export SSH_AUTH_SOCK;\n"
            "SSH_AGENT_PID=12345; export SSH_AGENT_PID;\n"
        )
        ...
        def staged_run(*args, **kwargs):  # noqa: ARG001
            call_count["n"] += 1
            if call_count["n"] == 1:
                # first call: ssh-agent succeeds
                return MagicMock(returncode=0, stdout=mock_agent_output, stderr="")
            # second call: ssh-add fails
            raise subprocess.CalledProcessError(
                returncode=1,
                cmd=["/usr/bin/ssh-add", "-"],
                output="",
                stderr="Error loading key '(stdin)': invalid format",
            )

        with (
            patch("subprocess.run", side_effect=staged_run),
            patch("os.kill"),
        ):
            secret_key = "-----BEGIN OPENSSH PRIVATE KEY-----\nSECRETMATERIAL\n-----END OPENSSH PRIVATE KEY-----"
            with pytest.raises(GitError) as exc_info:
                with setup_ssh_key(secret_key):
                    pass
            err = str(exc_info.value)
            assert "ssh-add failed" in err
            assert "invalid format" in err
            # critical: key body must not leak into the error message
            assert "SECRETMATERIAL" not in err

    def test_input_deploy_key_stripped_from_subprocess_env(self):
        """The deploy key passed via INPUT_DEPLOY_KEY env must not be
        inherited by any subprocess (ssh-agent, ssh-add, ...). Principle of
        least authority for child processes."""
        mock_agent_output = (
            "SSH_AUTH_SOCK=/tmp/ssh-mock/agent.12345; export SSH_AUTH_SOCK;\n"
            "SSH_AGENT_PID=12345; export SSH_AGENT_PID;\n"
        )
        ...
        def capture_run(*args, **kwargs):  # noqa: ARG001
            env = kwargs.get("env")
            if env is not None:
                captured_envs.append(env)
            return MagicMock(returncode=0, stdout=mock_agent_output, stderr="")

        secret_marker = "DO-NOT-LEAK-THIS-KEY-MATERIAL"
        with (
            patch.dict(os.environ, {"INPUT_DEPLOY_KEY": secret_marker}),
            patch("subprocess.run", side_effect=capture_run),
            patch("os.kill"),
        ):
            test_key = "-----BEGIN TEST-----\ntest\n-----END TEST-----"
        ...
        assert captured_envs, "expected at least one subprocess call with env"
        for env in captured_envs:
            assert "INPUT_DEPLOY_KEY" not in env, f"INPUT_DEPLOY_KEY leaked into subprocess env: {sorted(env.keys())}"
            # also verify the marker value isn't lurking under any other key
            assert secret_marker not in env.values()


class TestCleanEnv:
    """Tests for the _clean_env helper."""

    def test_strips_all_sensitive_vars(self):
        with patch.dict(os.environ, {name: "secret" for name in SENSITIVE_ENV_VARS}):
            env = _clean_env()
    ...
    def test_extra_overrides_existing(self):
        with patch.dict(os.environ, {"PATH": "/usr/bin"}):
            env = _clean_env({"PATH": "/opt/bin", "EXTRA": "value"})
            assert env["PATH"] == "/opt/bin"
            assert env["EXTRA"] == "value"

    def test_run_git_strips_sensitive_env(self):
        """End-to-end: run_git must not pass INPUT_DEPLOY_KEY to git."""
        captured: dict[str, dict[str, str] | None] = {"env": None}

        def capture(cmd, **kwargs):  # noqa: ARG001
            captured["env"] = kwargs.get("env")
            return MagicMock(returncode=0, stdout="", stderr="")

        with (
            patch.dict(os.environ, {"INPUT_DEPLOY_KEY": "leak-me"}),
        ...
        assert "INPUT_DEPLOY_KEY" not in env
...
class TestTagExistsRemotely:
    ...
    def test_nonexistent_tag(self, isolated_git_repo_with_origin: IsolatedGitRepo):
        remote = str(isolated_git_repo_with_origin.origin_path)
        assert tag_exists_remotely(remote, "definitely-not-a-real-tag-12345") is False
    ...
    def test_existing_tag(self, isolated_git_repo_with_origin: IsolatedGitRepo):
        ...
        remote = str(isolated_git_repo_with_origin.origin_path)
        assert tag_exists_remotely(remote, "v1.0.0") is True
    ...
    def test_does_not_match_partial(self, isolated_git_repo_with_origin: IsolatedGitRepo):
        ...
        subprocess.run(["git", "push", "origin", "v1.0.0"], check=True, capture_output=True)
        remote = str(isolated_git_repo_with_origin.origin_path)
        assert tag_exists_remotely(remote, "v1.0") is False

    def test_propagates_git_error(self):
        """If ls-remote fails (network/auth/etc), GitError must propagate so
        the caller doesn't fall through to push and mask the real cause."""
        with patch("create_tag.run_git") as mock_run_git:
            mock_run_git.side_effect = GitError("remote not found")
            with pytest.raises(GitError):
                tag_exists_remotely("bogus-remote", "v1.0.0")
...
class TestTagExistsRemotelyExactMatch:
    ...
    def test_exact_match_required(self):
        ...
        with patch("create_tag.run_git") as mock_run_git:
            ...
            assert tag_exists_remotely("origin", "v1.0") is False
    ...
    def test_finds_exact_tag(self):
        with patch("create_tag.run_git") as mock_run_git:
            ...
            assert tag_exists_remotely("origin", "v1.0.0") is True
    ...
    def test_empty_output(self):
        with patch("create_tag.run_git") as mock_run_git:
            ...
            assert tag_exists_remotely("origin", "v1.0.0") is False

    def test_env_is_passed_through(self):
        """Verify the env kwarg (carrying GIT_SSH_COMMAND) reaches run_git."""
        with patch("create_tag.run_git") as mock_run_git:
            mock_run_git.return_value = MagicMock(stdout="")
            tag_exists_remotely("origin", "v1.0.0", env={"GIT_SSH_COMMAND": "ssh -X"})
            _, kwargs = mock_run_git.call_args
            assert kwargs.get("env") == {"GIT_SSH_COMMAND": "ssh -X"}
...
class TestCreateAndPushTag:
    ...
    These tests use an isolated git repository because git operations need to
    run in a real repo. They never mutate global config and (post-refactor)
    they never mutate the repo's local config either.
    """
    ...
    def _snapshot_repo_config(self) -> dict[str, str]:
        """Capture every key/value in the repo-local .git/config."""
    ...
    def test_tag_already_exists_locally(
        ...
        """Existing local tag raises ValidationError and leaves repo config untouched."""
        subprocess.run(
            ["git", "-c", "tag.gpgsign=false", "tag", "v1.0.0"],
            check=True,
            capture_output=True,
        ...
        before = self._snapshot_repo_config()
        ...
        after = self._snapshot_repo_config()
        assert before == after, "create_and_push_tag must not mutate repo-local git config"
    ...
    def test_tag_already_exists_remotely(
        ...
        """Existing remote tag raises ValidationError and leaves repo config untouched."""
        before = self._snapshot_repo_config()
        with (
    ...
    def test_remote_check_failure_propagates(
        self,
        isolated_git_repo_with_origin: IsolatedGitRepo,
        valid_config: Config,
        mock_setup_ssh_key,
    ):
        """If the remote tag check itself errors (e.g., network), the GitError
        must bubble up — not get silently swallowed and treated as 'tag absent',
        which would then cause us to attempt a push and mask the real cause."""
        with (
            patch("create_tag.setup_ssh_key", mock_setup_ssh_key),
            patch("create_tag.tag_exists_remotely", side_effect=GitError("network down")),
        ):
            with pytest.raises(GitError) as exc_info:
                create_and_push_tag(valid_config)
            assert "network down" in str(exc_info.value)

    def test_success_path_uses_git_ssh_command_env_and_no_config_mutation(
        self,
        isolated_git_repo_with_origin: IsolatedGitRepo,
        mock_setup_ssh_key,
    ):
        """Verify: GIT_SSH_COMMAND is passed via env on push (not via repo config),
        the SSH URL is passed positionally to push (not via remote.origin.url),
        identity is supplied via `-c` overrides on tag creation, and the local
        .git/config is byte-identical before and after."""
        config = Config(
            tag="v2.0.0",
            deploy_key="-----BEGIN OPENSSH PRIVATE KEY-----\nbase64data\n-----END OPENSSH PRIVATE KEY-----",
        ...
        captured: dict[str, object] = {"push_args": None, "push_env": None, "tag_args": None}

        def mock_run_git_selective(args: list[str], env: dict[str, str] | None = None):
            # capture and short-circuit network-bound commands; let everything
            # else (rev-parse, ls-remote against local origin, etc.) run for real.
            if args and args[0] == "push":
                captured["push_args"] = args
                captured["push_env"] = env
                return MagicMock(returncode=0, stdout="", stderr="")
            # the `git -c user.name=... -c user.email=... tag -a ...` call has
            # `-c` as the first arg, not `tag`. Capture it for assertions.
            if "tag" in args and "-a" in args:
                captured["tag_args"] = args
        ...
        with (
            ...
            # ls-remote against a tag that doesn't exist returns empty — safe to let through,
            # but we mock to avoid the local-path push fallback if anything goes sideways.
            patch("create_tag.tag_exists_remotely", return_value=False),
        ):
            sha = create_and_push_tag(config)
        after = self._snapshot_repo_config()

        # 1. .git/config is byte-identical
        assert before == after, "create_and_push_tag must not mutate repo-local git config"

        # 2. push received the SSH URL positionally
        push_args = captured["push_args"]
        assert push_args is not None
        assert push_args[0] == "push"
        assert push_args[1] == "git@github.com:owner/repo.git"
        assert push_args[2] == "v2.0.0"

        # 3. push received GIT_SSH_COMMAND via env
        push_env = captured["push_env"]
        assert push_env is not None
        assert "GIT_SSH_COMMAND" in push_env
        assert "IdentityAgent=" in push_env["GIT_SSH_COMMAND"]
        assert "StrictHostKeyChecking=yes" in push_env["GIT_SSH_COMMAND"]

        # 4. tag command used -c overrides for identity (no `git config` mutation)
        tag_args = captured["tag_args"]
        assert tag_args is not None
        assert "-c" in tag_args
        assert "user.name=bot" in tag_args
        assert "user.email=bot@example.com" in tag_args

        # 5. SHA was returned and is a real 40-char hex (came from real rev-parse)
        assert re.match(r"^[a-f0-9]{40}$", sha)
...
class TestSecurityFeatures:
    ...
    def _capture_ssh_command(self, tmp_path: Path) -> str:
        """Helper: invoke create_and_push_tag with mocks and return the
        GIT_SSH_COMMAND seen by run_git for the network-bound call."""
        valid_config = Config(
            tag="v1.0.0",
            deploy_key="-----BEGIN OPENSSH PRIVATE KEY-----\nbase64data\n-----END OPENSSH PRIVATE KEY-----",
            tag_message="Release v1.0.0",
        ...
        def capture_run_git(args, env=None):  # noqa: ARG001
            # capture env from the push call specifically
            if args and args[0] == "push":
        ...
        with (
            patch("create_tag.setup_ssh_key", mock_setup_ssh_key),
            patch("create_tag.run_git", side_effect=capture_run_git),
            ...
            try:
                ...
                pass  # mocked environment won't fully complete

        env = captured_env["env"]
        assert env is not None, "push was never invoked"
        assert "GIT_SSH_COMMAND" in env, "GIT_SSH_COMMAND must be passed via env"
    ...
    def test_ssh_command_uses_strict_host_checking(self, tmp_path: Path):
        ...
        and is delivered via GIT_SSH_COMMAND env (never via repo config)."""
        ssh_command = self._capture_ssh_command(tmp_path)
        ...
        # using IdentityAgent (ssh-agent socket), not -i (file)
        assert "IdentityAgent=" in ssh_command
        assert "-i " not in ssh_command

    def test_ssh_command_paths_are_quoted(self, tmp_path: Path):
        """Verify paths in SSH command go through shlex.quote."""
        ssh_command = self._capture_ssh_command(tmp_path)
        assert "-o IdentityAgent=" in ssh_command
        assert "-o UserKnownHostsFile=" in ssh_command
...
class TestSSHCommandPathQuoting:
    ...
    def test_ssh_command_quotes_path_with_spaces(self, tmp_path: Path):
        """Verify paths with spaces are properly quoted in GIT_SSH_COMMAND."""
        ...
        # shlex.quote should have wrapped paths-with-spaces in single quotes
